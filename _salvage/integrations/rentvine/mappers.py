...
def map_bill(data):
    ...
    defaults = {
        "bill_type_id": _safe_int(_get(bill, "billTypeID", "billType_id", default=None)),
        "payee_contact_id": _safe_int(
            _get(bill, "payeeContactID", "payee_contact_id", default=None)
        ),
...
def map_bill_charge(data):
    ...
    if rentvine_transaction_id is None:
        raise ValueError(f"Transaction record missing transactionID: {list(tx.keys())[:5]}")
    ...
    defaults = {
        ...
        "amount_paid": _safe_decimal(_get(tx, "amountPaid", "amount_paid", default=None)),
        "primary_ledger_id": _safe_int(
            _get(tx, "primaryLedgerID", "primary_ledger_id", default=None)
        ),
        "property_id": _safe_int(_get(tx, "propertyID", "property_id", default=None)),
        "unit_id": _safe_int(_get(tx, "unitID", "unit_id", default=None)),
        "charge_account_id": _safe_int(
            _get(tx, "chargeAccountID", "charge_account_id", default=None)
        ),
        "description": str(_get(tx, "description", default="") or ""),
        "date_posted": _safe_date(_get(tx, "datePosted", "date_posted", default=None)),
