...
class PropertyMeldClient:
    ...
    def __init__(self, base_url=None, client_id=None, client_secret=None, management_id=None):
        config = getattr(settings, "PROPERTY_MELD", {})
        self.base_url = (
            base_url or config.get("BASE_URL", "https://api.propertymeld.com/api/v2")
        ).rstrip("/")
    ...
    def _authenticate(self):
        ...
        # Try the standard OAuth2 token endpoint (common patterns for DRF + simple-jwt)
        ...
        logger.info("Property Meld: authenticated successfully")

    def _ensure_auth(self):
        """Authenticate if no token or token is about to expire."""
        if not self._token or time.time() >= self._token_expires_at - self.TOKEN_EXPIRY_BUFFER:
            self._authenticate()
        ...
        for attempt in range(self.MAX_RETRIES + 1):
            try:
                ...
                if attempt < self.MAX_RETRIES:
                    ...
                    logger.warning(
                        "Property Meld request failed (attempt %d/%d), retrying in %ds: %s",
            ...
            if response.status_code == 401 and not reauthenticated:
                logger.info("Property Meld: 401 received, re-authenticating")
                self._authenticate()
            ...
            if response.status_code in self.RETRYABLE_STATUS_CODES:
                if attempt < self.MAX_RETRIES:
                    ...
                    if retry_after:
                        ...
                        wait = self.BACKOFF_FACTOR * (2 ** attempt)
                    logger.warning(
                        "Property Meld API %d (attempt %d/%d), retrying in %ds",
    ...
    def get_all(self, path, page_size=None):
        ...
        while True:
            ...
            logger.info(
                "Property Meld: fetched offset=%d from %s (%d records, %d total so far)",
                offset, path, len(records), len(all_records),
            )
        ...
        logger.info("Property Meld: fetched %d total records from %s", len(all_records), path)
        return all_records
