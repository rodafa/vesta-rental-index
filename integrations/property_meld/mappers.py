...
from integrations.utils import safe_date, safe_datetime

logger = logging.getLogger(__name__)


# ---------------------------------------------------------------------------
# Status sets — used by services and the maintenance email selector.
# ---------------------------------------------------------------------------
...
def map_meld(data):
    ...
    assigned_vendor_name = _extract_vendor_name(
        data.get("vendor_assignment_requests") or []
    )
    ...
    resident_presence_required = bool(
        data.get("tenant_presence_required") or False
    )
    ...
    # Date fields
    marked_complete = safe_datetime(data.get("marked_complete"))
    completion_date = safe_datetime(data.get("completion_date"))
    started = safe_datetime(data.get("started"))
    due_date = safe_date(data.get("due_date"))

    owner_approval_status = _normalise_owner_approval(
    ...
    source_created_at = safe_datetime(data.get("created"))
    source_modified_at = safe_datetime(data.get("updated"))
